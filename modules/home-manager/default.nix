    ...
    tmux = {
      ...
      extraConfig = ''
        ...
        bind-key -n C-f run-shell "sesh connect \"$(
            sesh list --hide-attached -t | fzf-tmux --ansi -p 55%,60% \
                ...
                --bind 'ctrl-f:change-prompt(🔎  )+reload(fd -H -d 2 -t d -E .Trash . ~/IdeaProjects ~/.dotfiles)' \
                --bind 'ctrl-d:execute(tmux kill-session -t {})+change-prompt(⚡  )+reload(sesh list --hide-attached -t)'
        )\""
